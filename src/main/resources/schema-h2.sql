...
-- SQ_VECTORX_SAMPLE_POST_01
CREATE SEQUENCE IF NOT EXISTS SQ_VECTORX_SAMPLE_POST_01 START WITH 1 INCREMENT BY 1;

-- 샘플 게시판
CREATE TABLE IF NOT EXISTS VECTORX_SAMPLE_POST
(
    POST_ID       INTEGER             NOT NULL,     -- 게시_아이디
    AUTH_NM       VARCHAR(100)        NOT NULL,     -- 작성자 아이디
    AUTH_EML      VARCHAR(100)        NOT NULL,     -- 작성자 이메일
    POST_TTL      VARCHAR(100)        NOT NULL,     -- 게시 제목
    POST_CONT     TEXT                NOT NULL,     -- 게시 내용
    VIEW_CNT      INTEGER             NOT NULL,     -- 조회_수
    REG_DTTM      TIMESTAMP           NOT NULL, -- 등록_일시
    REG_USR_ID    VARCHAR(20)         NULL,     -- 등록_사용자_아이디
    REG_USR_IP    VARCHAR(20)         NULL,     -- 등록_사용자_아이피
    UPD_DTTM      TIMESTAMP           NULL,     -- 수정_일시
...
COMMENT ON TABLE VECTORX_SAMPLE_POST IS 'VECTORX 샘플 게시';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.POST_ID IS '게시_아이디';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.AUTH_NM IS '작성자_아이디';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.AUTH_EML IS '작성자_이메일';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.POST_TTL IS '게시_제목';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.POST_CONT IS '게시_내용';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.VIEW_CNT IS '조회_수';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.REG_DTTM IS '등록_일시';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.REG_USR_ID IS '등록_사용자_아이디';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.REG_USR_IP IS '등록_사용자_아이피';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.UPD_DTTM IS '수정_일시';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.UPD_USR_ID IS '수정_사용자_아이디';
COMMENT ON COLUMN VECTORX_SAMPLE_POST.UPD_USR_IP IS '수정_사용자_아이피';

ALTER TABLE VECTORX_SAMPLE_POST ADD CONSTRAINT IF NOT EXISTS PK_VECTORX_SAMPLE_POST PRIMARY KEY(POST_ID);
