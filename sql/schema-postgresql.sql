...
CREATE UNIQUE INDEX PK_VECTORX_APP_STT
	ON VECTORX_APP_STT
	( -- 어플리케이션_상태
		APP_STT_ID ASC -- 어플리케이션_상태_아이디
	)
;
-- 어플리케이션_상태
ALTER TABLE VECTORX_APP_STT
	ADD CONSTRAINT PK_VECTORX_APP_STT
		 -- 어플리케이션_상태 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_APP_STT;

-- 어플리케이션_상태 기본키
COMMENT ON CONSTRAINT PK_VECTORX_APP_STT ON VECTORX_APP_STT IS '어플리케이션_상태 기본키';
...
ALTER TABLE VECTORX_APP_STT_MNU
	ADD CONSTRAINT PK_VECTORX_APP_STT_MNU
		 -- 어플리케이션_상태_메뉴 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_APP_STT_MNU;

-- 어플리케이션_상태_메뉴 기본키
COMMENT ON CONSTRAINT PK_VECTORX_APP_STT_MNU ON VECTORX_APP_STT_MNU IS '어플리케이션_상태_메뉴 기본키';
...
-- 권한 인덱스
COMMENT ON INDEX IX_VECTORX_AUTH IS '권한 인덱스';
...
-- 권한 인덱스2
COMMENT ON INDEX IX_VECTORX_AUTH2 IS '권한 인덱스2';
...
ALTER TABLE VECTORX_AUTH
	ADD CONSTRAINT PK_VECTORX_AUTH
		 -- 권한 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH;

-- 권한 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH ON VECTORX_AUTH IS '권한 기본키';
...
(
	...
	USE_SRT_DT          VARCHAR(8)    NULL,     -- 사용_시작_일자
	USE_END_DT          VARCHAR(8)    NULL,     -- 사용_종료_일자
	USR_AUTH_GRP_DEL_YN CHAR(1)       NULL     DEFAULT 'N', -- 사용자_권한_그룹_삭제_여부
	ADD_ROL_LIST_VAL    VARCHAR(1000) NULL,     -- 추가_역할_목록_값
	REG_DTTM            TIMESTAMP     NOT NULL, -- 등록_일시
	REG_USR_ID          VARCHAR(20)   NULL,     -- 등록_사용자_아이디
	REG_USR_IP          VARCHAR(20)   NULL,     -- 등록_사용자_아이피
...
CREATE UNIQUE INDEX PK_VECTORX_AUTH_APV
	ON VECTORX_AUTH_APV
	( -- 권한_승인
		USR_APV_ID ASC -- 사용자_승인_아이디
	)
;
-- 권한_승인
ALTER TABLE VECTORX_AUTH_APV
	ADD CONSTRAINT PK_VECTORX_AUTH_APV
		 -- 권한_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_APV;

-- 권한_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_APV ON VECTORX_AUTH_APV IS '권한_승인 기본키';
...
(
	...
	USE_YN             CHAR(1)       NULL,     -- 사용_여부
	DEL_YN             CHAR(1)       NULL     DEFAULT 'N', -- 삭제_여부
	PRV_TRT_YN         CHAR(1)       NULL     DEFAULT 'N', -- 개인정보_취급_여부
	SYS_CD             CHAR(2)       NULL,     -- 시스템_코드
	APV_GRP_YN         CHAR(1)       NULL     DEFAULT 'N', -- 승인_그룹_여부
	APV_NEC_YN         CHAR(1)       NULL,     -- 승인_필요_여부
...
ALTER TABLE VECTORX_AUTH_GRP
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP
		 -- 권한_그룹 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP;

-- 권한_그룹 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP ON VECTORX_AUTH_GRP IS '권한_그룹 기본키';
...
ALTER TABLE VECTORX_AUTH_GRP_APV
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_APV
		 -- 권한_그룹_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_APV;

-- 권한_그룹_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_APV ON VECTORX_AUTH_GRP_APV IS '권한_그룹_승인 기본키';
...
ALTER TABLE VECTORX_AUTH_GRP_APV_STEP
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_APV_STEP
		 -- 권한_그룹_승인_단계 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_APV_STEP;

-- 권한_그룹_승인_단계 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_APV_STEP ON VECTORX_AUTH_GRP_APV_STEP IS '권한_그룹_승인_단계 기본키';
...
ALTER TABLE VECTORX_AUTH_GRP_APV_USR
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_APV_USR
		 -- 권한_그룹_승인_사용자 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_APV_USR;

-- 권한_그룹_승인_사용자 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_APV_USR ON VECTORX_AUTH_GRP_APV_USR IS '권한_그룹_승인_사용자 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP
	...
	( -- 권한_그룹_승인_사용자_요청_대상_그룹
		USR_APV_ID ASC, -- 사용자_승인_아이디
		AUTH_GRP_ID ASC, -- 권한_그룹_아이디
		REQ_AUTH_GRP_ID ASC -- 요청_권한_그룹_아이디
	)
;
-- 권한_그룹_승인_사용자_요청_대상_그룹
ALTER TABLE VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP
		 -- 권한_그룹_승인_사용자_요청_대상_그룹 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP;

-- 권한_그룹_승인_사용자_요청_대상_그룹 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP ON VECTORX_AUTH_GRP_APV_USR_REQ_TGT_GRP IS '권한_그룹_승인_사용자_요청_대상_그룹 Primary key';
...
(
	...
	APV_NEC_YN         CHAR(1)       NULL,     -- 승인_필요_여부
	AUTH_GRP_STEP_NO   INTEGER       NULL,     -- 권한_그룹_단계_번호
	WORK_DV_CD         CHAR(1)       NULL,     -- 업무_구분_코드
	ADD_ROL_LIST_VAL   VARCHAR(1000) NULL,     -- 추가_역할_목록_값
	USE_DV_CD_LIST_VAL VARCHAR(500)  NULL,     -- 사용_구분_코드_목록_값
	REG_DTTM           TIMESTAMP     NOT NULL, -- 등록_일시
	REG_USR_ID         VARCHAR(20)   NULL,     -- 등록_사용자_아이디
...
CREATE UNIQUE INDEX PK_VECTORX_AUTH_GRP_HIST
	ON VECTORX_AUTH_GRP_HIST
	( -- 권한_그룹_이력
		AUTH_GRP_HIST_ID ASC -- 권한_그룹_이력_아이디
	)
;
-- 권한_그룹_이력
ALTER TABLE VECTORX_AUTH_GRP_HIST
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_HIST
		 -- 권한_그룹_이력 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_HIST;

-- 권한_그룹_이력 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_HIST ON VECTORX_AUTH_GRP_HIST IS '권한_그룹_이력 Primary key';
...
ALTER TABLE VECTORX_AUTH_GRP_USR
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_USR
		 -- 권한_그룹_사용자 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_USR;

-- 권한_그룹_사용자 기본키
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_USR ON VECTORX_AUTH_GRP_USR IS '권한_그룹_사용자 기본키';
...
ALTER TABLE VECTORX_AUTH_GRP_USR_HIST
	ADD CONSTRAINT PK_VECTORX_AUTH_GRP_USR_HIST
		 -- 권한_그룹_사용자_이력 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_GRP_USR_HIST;

-- 권한_그룹_사용자_이력 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_GRP_USR_HIST ON VECTORX_AUTH_GRP_USR_HIST IS '권한_그룹_사용자_이력 Primary key';
...
ALTER TABLE VECTORX_AUTH_GRP_USR_REQ_TGT_GRP
	ADD CONSTRAINT PK_USTR_ATH_GRP_RQ_TGT_GRP
		 -- 권한_그룹_사용자_요청_대상_그룹 Primary key
	PRIMARY KEY
	USING INDEX PK_USTR_ATH_GRP_RQ_TGT_GRP;

-- 권한_그룹_사용자_요청_대상_그룹 Primary key
COMMENT ON CONSTRAINT PK_USTR_ATH_GRP_RQ_TGT_GRP ON VECTORX_AUTH_GRP_USR_REQ_TGT_GRP IS '권한_그룹_사용자_요청_대상_그룹 Primary key';
...
CREATE UNIQUE INDEX PK_VECTORX_AUTH_HIST
	ON VECTORX_AUTH_HIST
	( -- 권한_이력
		AUTH_HIST_ID ASC -- 권한_이력_아이디
	)
;
-- 권한_이력
ALTER TABLE VECTORX_AUTH_HIST
	ADD CONSTRAINT PK_VECTORX_AUTH_HIST
		 -- 권한_이력 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_AUTH_HIST;

-- 권한_이력 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_AUTH_HIST ON VECTORX_AUTH_HIST IS '권한_이력 Primary key';
...
ALTER TABLE VECTORX_BAT_MNGR
	ADD CONSTRAINT PK_VECTORX_BAT_MNGR
		 -- 배치_매니저 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_BAT_MNGR;

-- 배치_매니저 기본키
COMMENT ON CONSTRAINT PK_VECTORX_BAT_MNGR ON VECTORX_BAT_MNGR IS '배치_매니저 기본키';
...
(
	...
	USE_YN      CHAR(1)       NULL,     -- 사용_여부
	WIP_PLCY_CD CHAR(2)       NULL,     -- 작업중_정책_코드
...
ALTER TABLE VECTORX_BAT_TASK
	ADD CONSTRAINT PK_VECTORX_BAT_TASK
		 -- 배치_작업 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_BAT_TASK;

-- 배치_작업 기본키
COMMENT ON CONSTRAINT PK_VECTORX_BAT_TASK ON VECTORX_BAT_TASK IS '배치_작업 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_BAT_TASK_EXEC
	...
	( -- 배치_작업_실행
		TASK_EXEC_ID ASC -- 작업_실행_아이디
	)
;
-- 배치_작업_실행
ALTER TABLE VECTORX_BAT_TASK_EXEC
	ADD CONSTRAINT PK_VECTORX_BAT_TASK_EXEC
		 -- 배치_작업_실행 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_BAT_TASK_EXEC;

-- 배치_작업_실행 기본키
COMMENT ON CONSTRAINT PK_VECTORX_BAT_TASK_EXEC ON VECTORX_BAT_TASK_EXEC IS '배치_작업_실행 기본키';
...
ALTER TABLE VECTORX_BAT_WRKR
	ADD CONSTRAINT PK_VECTORX_BAT_WRKR
		 -- 배치_워커 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_BAT_WRKR;

-- 배치_워커 기본키
COMMENT ON CONSTRAINT PK_VECTORX_BAT_WRKR ON VECTORX_BAT_WRKR IS '배치_워커 기본키';
...
ALTER TABLE VECTORX_CMM_CD
	ADD CONSTRAINT PK_VECTORX_CMM_CD
		 -- 공통_코드 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_CMM_CD;

-- 공통_코드 기본키
COMMENT ON CONSTRAINT PK_VECTORX_CMM_CD ON VECTORX_CMM_CD IS '공통_코드 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_DBMS_QRY_HIST
	ON VECTORX_DBMS_QRY_HIST
	( -- DBMS_쿼리_이력
		DBMS_QRY_HIST_ID ASC -- DBMS_쿼리_이력_아이디
	)
;
-- DBMS_쿼리_이력
ALTER TABLE VECTORX_DBMS_QRY_HIST
	ADD CONSTRAINT PK_VECTORX_DBMS_QRY_HIST
		 -- DBMS_쿼리_이력 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_DBMS_QRY_HIST;

-- DBMS_쿼리_이력 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_DBMS_QRY_HIST ON VECTORX_DBMS_QRY_HIST IS 'DBMS_쿼리_이력 Primary key';
...
ALTER TABLE VECTORX_FILES
	ADD CONSTRAINT PK_VECTORX_FILES
		 -- 파일 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_FILES;

-- 파일 기본키
COMMENT ON CONSTRAINT PK_VECTORX_FILES ON VECTORX_FILES IS '파일 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_FILE_GRP
	ON VECTORX_FILE_GRP
	( -- 파일_그룹
		FILE_GRP_ID ASC -- 파일_그룹_아이디
	)
;
-- 파일_그룹
ALTER TABLE VECTORX_FILE_GRP
	ADD CONSTRAINT PK_VECTORX_FILE_GRP
		 -- 파일_그룹 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_FILE_GRP;

-- 파일_그룹 기본키
COMMENT ON CONSTRAINT PK_VECTORX_FILE_GRP ON VECTORX_FILE_GRP IS '파일_그룹 기본키';
...
ALTER TABLE VECTORX_IFS
	ADD CONSTRAINT PK_VECTORX_IFS
		 -- 인터페이스 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_IFS;

-- 인터페이스 기본키
COMMENT ON CONSTRAINT PK_VECTORX_IFS ON VECTORX_IFS IS '인터페이스 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_IFS_HIST
	ON VECTORX_IFS_HIST
	( -- 인터페이스_이력
		IF_HIST_ID ASC -- 인터페이스_이력_아이디
	)
;
-- 인터페이스_이력
ALTER TABLE VECTORX_IFS_HIST
	ADD CONSTRAINT PK_VECTORX_IFS_HIST
		 -- 인터페이스_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_IFS_HIST;

-- 인터페이스_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_IFS_HIST ON VECTORX_IFS_HIST IS '인터페이스_이력 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_IFS_REQ_CTRL
	...
	( -- 인터페이스_요청_제어
		REQ_CTRL_ID ASC -- 요청_제어_아이디
	)
;
-- 인터페이스_요청_제어
ALTER TABLE VECTORX_IFS_REQ_CTRL
	ADD CONSTRAINT PK_VECTORX_IFS_REQ_CTRL
		 -- 인터페이스_요청_제어 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_IFS_REQ_CTRL;

-- 인터페이스_요청_제어 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_IFS_REQ_CTRL ON VECTORX_IFS_REQ_CTRL IS '인터페이스_요청_제어 Primary key';
...
ALTER TABLE VECTORX_IFS_REQ_CTRL_CHNL
	ADD CONSTRAINT PK_VECTORX_IFS_REQ_CTRL_CHNL
		 -- 인터페이스_요청_제어_채널 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_IFS_REQ_CTRL_CHNL;

-- 인터페이스_요청_제어_채널 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_IFS_REQ_CTRL_CHNL ON VECTORX_IFS_REQ_CTRL_CHNL IS '인터페이스_요청_제어_채널 Primary key';
...
ALTER TABLE VECTORX_IF_CHNL
	ADD CONSTRAINT PK_VECTORX_IF_CHNL
		 -- 인터페이스_채널 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_IF_CHNL;

-- 인터페이스_채널 기본키
COMMENT ON CONSTRAINT PK_VECTORX_IF_CHNL ON VECTORX_IF_CHNL IS '인터페이스_채널 기본키';
...
ALTER TABLE VECTORX_IF_KEY
	ADD CONSTRAINT PK_VECTORX_IF_KEY
		 -- 인터페이스_키 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_IF_KEY;

-- 인터페이스_키 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_IF_KEY ON VECTORX_IF_KEY IS '인터페이스_키 Primary key';
...
CREATE UNIQUE INDEX PK_VECTORX_LOGIN_HIST
	ON VECTORX_LOGIN_HIST
	( -- 로그인_이력
		LOGIN_HIST_ID ASC -- 로그인_이력_아이디
	)
;
-- 로그인_이력
ALTER TABLE VECTORX_LOGIN_HIST
	ADD CONSTRAINT PK_VECTORX_LOGIN_HIST
		 -- 로그인_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_LOGIN_HIST;

-- 로그인_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_LOGIN_HIST ON VECTORX_LOGIN_HIST IS '로그인_이력 기본키';
...
(
	...
	MNU_TY_CD       CHAR(2)       NULL,     -- 메뉴_유형_코드
	USE_YN          CHAR(1)       NOT NULL, -- 사용_여부
	DEL_YN          CHAR(1)       NULL     DEFAULT 'N', -- 삭제_여부
	MNU_STEP_NO     INTEGER       NOT NULL DEFAULT 1, -- 메뉴_단계_번호
	IP_LMT_YN       CHAR(1)       NULL     DEFAULT 'N', -- 아이피제한여부
	IP_LIST         VARCHAR(1000) NULL,     -- 아이피_목록
	PRV_VIEW_YN     CHAR(1)       NULL     DEFAULT 'N', -- 개인정보_조회_여부
	DP_YN           CHAR(1)       NULL,     -- 전시_여부
	AUTH_SCOP_CD    CHAR(2)       NULL,     -- 권한_범위_코드
...
ALTER TABLE VECTORX_MNU
	ADD CONSTRAINT PK_VECTORX_MNU
		 -- 메뉴 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU;

-- 메뉴 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU ON VECTORX_MNU IS '메뉴 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_MNU_ACC_HIST
	ON VECTORX_MNU_ACC_HIST
	( -- 메뉴_접근_이력
		MNU_ACC_ID ASC -- 메뉴_접근_아이디
	)
;
-- 메뉴_접근_이력
ALTER TABLE VECTORX_MNU_ACC_HIST
	ADD CONSTRAINT PK_VECTORX_MNU_ACC_HIST
		 -- 메뉴_접근_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_ACC_HIST;

-- 메뉴_접근_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_ACC_HIST ON VECTORX_MNU_ACC_HIST IS '메뉴_접근_이력 기본키';
...
ALTER TABLE VECTORX_MNU_AUTH
	ADD CONSTRAINT PK_VECTORX_MNU_AUTH
		 -- 메뉴_권한 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_AUTH;

-- 메뉴_권한 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_AUTH ON VECTORX_MNU_AUTH IS '메뉴_권한 기본키';
...
ALTER TABLE VECTORX_MNU_AUTH_APV
	ADD CONSTRAINT PK_VECTORX_MNU_AUTH_APV
		 -- 메뉴_권한_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_AUTH_APV;

-- 메뉴_권한_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_AUTH_APV ON VECTORX_MNU_AUTH_APV IS '메뉴_권한_승인 기본키';
...
ALTER TABLE VECTORX_MNU_AUTH_HIST
	ADD CONSTRAINT PK_VECTORX_MNU_AUTH_HIST
		 -- 메뉴_권한_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_AUTH_HIST;

-- 메뉴_권한_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_AUTH_HIST ON VECTORX_MNU_AUTH_HIST IS '메뉴_권한_이력 기본키';
...
ALTER TABLE VECTORX_MNU_FNC
	ADD CONSTRAINT PK_VECTORX_MNU_FNC
		 -- 메뉴_기능 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_FNC;

-- 메뉴_기능 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_FNC ON VECTORX_MNU_FNC IS '메뉴_기능 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_MNU_FNC_ACC_HIST
	ON VECTORX_MNU_FNC_ACC_HIST
	( -- 메뉴_기능_접근_이력
		MNU_FNC_ACC_ID ASC -- 메뉴_기능_접근_아이디
	)
;
-- 메뉴_기능_접근_이력
ALTER TABLE VECTORX_MNU_FNC_ACC_HIST
	ADD CONSTRAINT PK_VECTORX_MNU_FNC_ACC_HIST
		 -- 메뉴_기능_접근_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_FNC_ACC_HIST;

-- 메뉴_기능_접근_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_FNC_ACC_HIST ON VECTORX_MNU_FNC_ACC_HIST IS '메뉴_기능_접근_이력 기본키';
...
ALTER TABLE VECTORX_MNU_FNC_AUTH
	ADD CONSTRAINT PK_VECTORX_MNU_FNC_AUTH
		 -- 메뉴_기능_권한 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_FNC_AUTH;

-- 메뉴_기능_권한 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_FNC_AUTH ON VECTORX_MNU_FNC_AUTH IS '메뉴_기능_권한 기본키';
...
ALTER TABLE VECTORX_MNU_FNC_AUTH_APV
	ADD CONSTRAINT PK_VECTORX_MNU_FNC_AUTH_APV
		 -- 메뉴_기능_권한_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_FNC_AUTH_APV;

-- 메뉴_기능_권한_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_FNC_AUTH_APV ON VECTORX_MNU_FNC_AUTH_APV IS '메뉴_기능_권한_승인 기본키';
...
ALTER TABLE VECTORX_MNU_FNC_AUTH_HIST
	ADD CONSTRAINT PK_VECTORX_MNU_FNC_AUTH_HIST
		 -- 메뉴_기능_권한_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_MNU_FNC_AUTH_HIST;

-- 메뉴_기능_권한_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_MNU_FNC_AUTH_HIST ON VECTORX_MNU_FNC_AUTH_HIST IS '메뉴_기능_권한_이력 기본키';
...
(
	...
	TOP_FIX_YN    CHAR(1)            NULL     DEFAULT 'N', -- 상단_고정_여부
	...
	POP_POST_YN   CHAR(1)            NULL     DEFAULT 'N', -- 팝업_게시_여부
...
ALTER TABLE VECTORX_POSTS
	ADD CONSTRAINT PK_VECTORX_POSTS
		 -- 게시 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_POSTS;

-- 게시 기본키
COMMENT ON CONSTRAINT PK_VECTORX_POSTS ON VECTORX_POSTS IS '게시 기본키';
...
(
	...
	ATTCH_FILE_USE_YN  CHAR(1)     NULL,     -- 첨부_파일_사용_여부
	HTML_EDITOR_USE_YN CHAR(1)     NULL,     -- HTML_에디터_사용_여부
	POST_TERM_USE_YN   CHAR(1)     NULL,     -- 게시_기간_사용_여부
	TOP_FIX_USE_YN     CHAR(1)     NULL     DEFAULT 'N', -- 상단_고정_사용_여부
	POP_USE_YN         CHAR(1)     NULL     DEFAULT 'N', -- 팝업_사용_여부
	POST_DV_USE_YN     CHAR(1)     NULL,     -- 게시_구분_사용_여부
	POST__USE_YN       CHAR(1)     NULL,     -- 게시_태그_사용_여부
	USE_YN             CHAR(1)     NULL,     -- 사용_여부
	DEL_YN             CHAR(1)     NULL     DEFAULT 'N', -- 삭제_여부
	REG_DTTM           TIMESTAMP   NOT NULL, -- 등록_일시
	REG_USR_ID         VARCHAR(20) NULL,     -- 등록_사용자_아이디
...
CREATE UNIQUE INDEX PK_VECTORX_POST_GRP
	ON VECTORX_POST_GRP
	( -- 게시_그룹
		POST_GRP_ID ASC -- 게시_그룹_아이디
	)
;
-- 게시_그룹
ALTER TABLE VECTORX_POST_GRP
	ADD CONSTRAINT PK_VECTORX_POST_GRP
		 -- 게시_그룹 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_POST_GRP;

-- 게시_그룹 기본키
COMMENT ON CONSTRAINT PK_VECTORX_POST_GRP ON VECTORX_POST_GRP IS '게시_그룹 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_POST_OPN
	ON VECTORX_POST_OPN
	( -- 게시_의견
		POST_OPN_ID ASC -- 게시_의견_아이디
	)
;
-- 게시_의견
ALTER TABLE VECTORX_POST_OPN
	ADD CONSTRAINT PK_VECTORX_POST_OPN
		 -- 게시_의견 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_POST_OPN;

-- 게시_의견 기본키
COMMENT ON CONSTRAINT PK_VECTORX_POST_OPN ON VECTORX_POST_OPN IS '게시_의견 기본키';

-- 게시_태그
CREATE TABLE VECTORX_POST_TAG
(
	POST_ID    INTEGER     NULL,     -- 게시_아이디
	TAG_NM     VARCHAR(50) NULL,     -- 태그_명
	REG_DTTM   TIMESTAMP   NOT NULL, -- 등록_일시
	REG_USR_ID VARCHAR(20) NULL,     -- 등록_사용자_아이디
	REG_USR_IP VARCHAR(20) NULL,     -- 등록_사용자_아이피
	UPD_DTTM   TIMESTAMP   NULL,     -- 수정_일시
...
CREATE UNIQUE INDEX PK_VECTORX_PRV_ACC_HIST
	...
	( -- 개인정보_접근_이력
		PRV_ACC_ID ASC -- 개인정보_접근_아이디
	)
;
-- 개인정보_접근_이력
ALTER TABLE VECTORX_PRV_ACC_HIST
	ADD CONSTRAINT PK_VECTORX_PRV_ACC_HIST
		 -- 개인정보_접근_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_PRV_ACC_HIST;

-- 개인정보_접근_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_PRV_ACC_HIST ON VECTORX_PRV_ACC_HIST IS '개인정보_접근_이력 기본키';
...
CREATE UNIQUE INDEX PK_VECTORX_TOKEN
	ON VECTORX_TOKEN
	( -- 토큰
		REFRESH_TOKEN ASC -- 리프레시_토큰
	)
;
-- 토큰
ALTER TABLE VECTORX_TOKEN
	ADD CONSTRAINT PK_VECTORX_TOKEN
		 -- 토큰 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_TOKEN;

-- 토큰 기본키
COMMENT ON CONSTRAINT PK_VECTORX_TOKEN ON VECTORX_TOKEN IS '토큰 기본키';
...
ALTER TABLE VECTORX_USR
	ADD CONSTRAINT PK_VECTORX_USR
		 -- 사용자 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR;

-- 사용자 기본키
COMMENT ON CONSTRAINT PK_VECTORX_USR ON VECTORX_USR IS '사용자 기본키';
...
(
	...
	USR_APV_TY_CD    CHAR(2)       NULL,     -- 사용자_승인_유형_코드
	APV_STT_CD       CHAR(2)       NULL,     -- 승인_상태_코드
...
ALTER TABLE VECTORX_USR_APV
	ADD CONSTRAINT PK_VECTORX_USR_APV
		 -- 사용자_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR_APV;

-- 사용자_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_USR_APV ON VECTORX_USR_APV IS '사용자_승인 기본키';
...
ALTER TABLE VECTORX_USR_APV_STEP
	ADD CONSTRAINT PK_VECTORX_USR_APV_STEP
		 -- 사용자_승인_단계 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR_APV_STEP;

-- 사용자_승인_단계 기본키
COMMENT ON CONSTRAINT PK_VECTORX_USR_APV_STEP ON VECTORX_USR_APV_STEP IS '사용자_승인_단계 기본키';
...
(
	...
	USE_SRT_DT       VARCHAR(8)    NULL,     -- 사용_시작_일자
	USE_END_DT       VARCHAR(8)    NULL,     -- 사용_종료_일자
	USE_DV_CD        CHAR(2)       NULL,     -- 사용_구분_코드
	LST_LOGIN_DTTM   TIMESTAMP     NULL,     -- 최종_로그인_일시
	LOGIN_FAIL_CNT   INTEGER       NULL     DEFAULT 0, -- 로그인_실패_건수
	PWD_RESET_YN     CHAR(1)       NULL     DEFAULT 'Y', -- 비밀번호_초기화_여부
	IP_LMT_YN        CHAR(1)       NULL     DEFAULT 'N', -- 아이피_제한_여부
...
ALTER TABLE VECTORX_USR_HIST
	ADD CONSTRAINT PK_VECTORX_USR_HIST
		 -- 사용자_이력 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR_HIST;

-- 사용자_이력 기본키
COMMENT ON CONSTRAINT PK_VECTORX_USR_HIST ON VECTORX_USR_HIST IS '사용자_이력 기본키';
...
(
	USR_ID     VARCHAR(20)        NOT NULL, -- 사용자_아이디
	MNU_ID     INTEGER            NOT NULL, -- 메뉴_아이디
	DEF_MNU_YN CHAR(1)            NULL,     -- 기본_메뉴_여부
	SRT_NO     INTEGER            NULL,     -- 정렬_번호
	REG_DTTM   TIMESTAMP          NOT NULL, -- 등록_일시
	SYS_CD     VARCHAR(20)        NULL,     -- 시스템_코드
	REG_USR_ID VARCHAR(20)        NULL,     -- 등록_사용자_아이디
...
ALTER TABLE VECTORX_USR_MNU
	ADD CONSTRAINT PK_VECTORX_USR_MNU
		 -- 사용자_메뉴 Primary key
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR_MNU;

-- 사용자_메뉴 Primary key
COMMENT ON CONSTRAINT PK_VECTORX_USR_MNU ON VECTORX_USR_MNU IS '사용자_메뉴 Primary key';
...
(
	...
	PWD_RESET_YN CHAR(1)            NULL     DEFAULT 'N', -- 비밀번호_초기화_여부
	REG_DTTM     TIMESTAMP          NOT NULL, -- 등록_일시
	REG_USR_ID   VARCHAR(20)        NULL,     -- 등록_사용자_아이디
	REG_USR_IP   VARCHAR(20)        NULL,     -- 등록_사용자_아이피
	UPD_DTTM     TIMESTAMP          NULL,     -- 수정_일시
...
ALTER TABLE VECTORX_USR_STT_APV
	ADD CONSTRAINT PK_VECTORX_USR_STT_APV
		 -- 사용자_상태_승인 기본키
	PRIMARY KEY
	USING INDEX PK_VECTORX_USR_STT_APV;

-- 사용자_상태_승인 기본키
COMMENT ON CONSTRAINT PK_VECTORX_USR_STT_APV ON VECTORX_USR_STT_APV IS '사용자_상태_승인 기본키';
